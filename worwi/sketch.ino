...
{
    digitalWrite(rgbPins[lane][0], R ? LOW : HIGH);
    digitalWrite(rgbPins[lane][1], G ? LOW : HIGH);
    digitalWrite(rgbPins[lane][2], B ? LOW : HIGH);
}
...
void loop()
{
    ...
    for (int i = 0; i < LANES; i++)
    {
        ...
        if (distance[i] != -1 && distance[i] < minDistance)
        {
    ...
    for (int i = 0; i < LANES; i++)
    {
        ...
        else
        {
            ...
            if (i == chosenLane)
            {
                setRGB(i, true, false, false); // Red
            }
            else
            {
                setRGB(i, false, true, false); // Green
            }
        }
    }
